...
class ExternalPressureCalculator:
    ...
    def _gen_A_curves(self):
        for m in self.Do_t_values:
            nn = 1.1 * np.sqrt(m)
            n1, n2 = np.linspace(0.05, nn, 100), np.linspace(nn, 50, 100)
    ...
    def _load_B_files_or_raise(self):
        ...
        if missing: raise FileNotFoundError(f"缺少 B–A 数据文件：{', '.join(missing)}")
        for fn in self.file_names:
            d = np.loadtxt(os.path.join(self.ba_dir, fn))
            order = np.argsort(d[:, 1])
            d = d[order, :]
            _, uniq_idx = np.unique(d[:, 1], return_index=True)
            self.all_data.append(d[np.sort(uniq_idx), :])

    def Et(self, T):
        return float(np.interp(np.clip(T, self.temps[0], self.temps[-1]), self.temps, self.E_values))

    def A_value(self, D0, de, L):
        Do_de = np.clip(D0 / float(de), 4, 1000)
        L_Do = np.clip(L / float(D0), 0.05, 50.0)
        y_lines = [interpolate.interp1d(ld, ad, bounds_error=False, fill_value=(ad[0], ad[-1]))(L_Do) for ld, ad in
                   zip(self.L_Do_data, self.A_data)]
        return float(np.interp(Do_de, self.Do_t_values, y_lines))

    def B_value(self, A, T):
        logA = np.log10(max(float(A), 1e-12))
...
def B_from_p(D0, de, As, Ls, p): return p * D0 / (de + As / max(Ls, 1e-9))


def effective_b(D0, de): return 1.1 * np.sqrt(D0 * de)
...
def check_geom_bounds(rib_type, **kw):
    if rib_type == 'rect': return (16 <= kw['hs1'] <= 200) and (3 <= kw['ds1'] <= 60)
    if rib_type == 'angle': return (kw['ds1'] == kw['ds2']) and (20 <= kw['hs2'] <= 250) and (
                16 <= kw['hs1'] <= 250) and (3 <= kw['ds1'] <= 35)
    if rib_type == 'tee': return (20 <= kw['hs2'] <= 450) and (10 <= kw['hs1'] <= 150) and (3 <= kw['ds1'] <= 35) and (
                3 <= kw['ds2'] <= 35)
    return False
...
class PressureVesselProblemVec(Problem):
    def __init__(self, vessel_params, fixed=None):
        ...
        if fixed and 'rib_type_idx' in fixed: xl[2] = xu[2] = float(int(fixed['rib_type_idx']))
    ...
    while cur_gen < max_gen:
        ...
        if not seen_feasible and cur_best_F < PENALTY_VALUE:
            ...
            if cur_best_F < best_F - min_delta:
                best_F = cur_best_F; best_gen = cur_gen; no_improve = 0
            else:
                no_improve += 1
